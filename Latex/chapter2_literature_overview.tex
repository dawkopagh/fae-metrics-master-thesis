...
Current literature broadly categorizes these methods based on their access to the model's internal parameters: Gradient-based (White-box) and Perturbation-based (Black-box) approaches.
\studentinline{link relevant citation above}
...
\subsection{Localization metrics - pointing game and relevance mass}

\studentinline{TODO: Fill subsection on localizatiion metrics}
...
\subsubsection{Deletion and Insertion (AUC)}
Proposed by Petsiuk et al., these metrics measure the degradation (or improvement) of the model's score as features are removed (or added) based on their importance ranking \cite{petsiuk2018rise}.
\begin{itemize}
    \item \textbf{Deletion (AOPC):} Measures the Area Over the Perturbation Curve. A steeper drop implies the method correctly identified critical features.
    \item \textbf{Insertion:} Measures the Area Under the Curve as features are introduced to a baseline canvas.
\end{itemize}
The Deletion score for an image $x$ with $N$ pixels is calculated as:
\begin{equation}
    \text{Deletion}(x) = \sum_{k=1}^{N} (f(x) - f(x^{(k)}))
\end{equation}
where $x^{(k)}$ is the image with the top $k$ most important pixels masked.

\subsubsection{Infidelity}
Yeh et al. formulated Infidelity as the expected mean squared error between a perturbation in the input space $I$ and the dot product of the perturbation with the explanation $E$ \cite{hedstrom2023quantus}. A faithful explanation should satisfy $I^T E \approx f(x) - f(x - I)$.
\begin{equation}
    \text{Infidelity} = \mathbb{E}_{I \sim \mu} \left[ (I^T E - (f(x) - f(x - I)))^2 \right]
\end{equation}
...
\subsubsection{Local Lipschitz Estimate}
Based on the Lipschitz continuity condition, this metric finds the maximum change in the explanation $\|\Phi(x) - \Phi(x')\|$ relative to the change in input $\|x - x'\|$ within a neighborhood $\mathcal{N}_\epsilon(x)$ \cite{alvarez2018robustness}:
\begin{equation}
    L(x) = \max_{x' \in \mathcal{N}_\epsilon(x)} \frac{\|\Phi(x) - \Phi(x')\|_2}{\|x - x'\|_2}
\end{equation}
A lower Lipschitz constant indicates a more stable and robust explanation method.
...
\begin{itemize}
    \item \textbf{Sparsity:} Measures the fraction of features with zero or near-zero attribution.
    \item \textbf{Effective Complexity:} Often calculated using the Shannon entropy of the normalized attribution distribution. High entropy indicates a diffuse explanation, while low entropy indicates a focused, sparse explanation.
\end{itemize}
...
signals receive higher weight without manual tuning. This mechanism provides
the theoretical foundation for the evaluation framework proposed in
Chapter~\ref{ch:methodology}, which extends the Autoweighted concept by
integrating the meta-evaluation criterion of \citet{hedstrom2024metaquantus}
into the weight derivation: a metric's weight is discounted not only by its
ensemble disagreement but also by its MetaQuantus failure rate.
