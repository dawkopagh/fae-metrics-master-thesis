...
Gradient-based methods exploit the differentiable nature of neural networks. They require access to the model's weights and gradients, operating on the intuition that the gradient of the output with respect to the input represents the model's sensitivity to feature changes.



\subsubsection{Saliency Maps }
The foundational approach, proposed by Simonyan et al., computes the gradient of the class score $S_c$ with respect to the input image $I$ \cite{selvaraju2017grad}. The relevance map is defined as:
\begin{equation}
...
Perturbation-based methods operate under a model-agnostic paradigm. They estimate feature importance by systematically modifying (perturbing) the input and observing the resulting changes in the model's output prediction.
...
\end{equation}
SHAP is the only method that satisfies the axioms of \textit{Efficiency}, \textit{Symmetry}, \textit{Dummy}, and \textit{Additivity}. However, exact computation is NP-hard, leading to approximation methods like GradientSHAP or KernelSHAP, which reintroduce estimation noise.
...
\end{equation}
The metric is typically averaged across the test set to produce a hit rate. Its binary nature makes it interpretable but coarse: a method that assigns high attribution to many pixels inside the mask but whose global maximum lies just outside receives a score of zero \cite{MISSING-pg}.
\studentinline{VERIFY citation: find Zhou et al.\ 2016 Pointing Game paper (Learning Deep Features for Discriminative Localization).}
...
\begin{itemize}
    \item \textbf{Failure of Guided Backpropagation:} The study revealed that methods like Guided Backpropagation and DeconvNet produced nearly identical heatmaps for a trained model and a random network. This indicates these methods act as edge detectors independent of the model's learned representations.
...
To rigorously assess FAE models, recent literature proposes a modular taxonomy of metrics. This thesis adopts the framework formalized by Quantus \cite{hedstrom2023quantus}, focusing on Faithfulness, Robustness, and Complexity.
...
Pixel Flipping, introduced by Samek et al., evaluates faithfulness by iteratively removing the most important features (as ranked by the attribution map) and measuring the resulting degradation in the model's output \cite{MISSING-pixelflip}.
\studentinline{VERIFY citation: Samek et al.\ 2017 --- ``Evaluating the Visualization of What a Deep Neural Network Has Learned.''}
Pixels are replaced in descending order of attribution magnitude with a baseline value (e.g., black). The metric aggregates the output change across all removal steps into an area-under-the-curve (AUC) score:
\begin{equation}
    \text{PixelFlipping}(x) = \frac{1}{N} \sum_{k=1}^{N} \bigl(f(x) - f(x^{(k)})\bigr),
\end{equation}
where $x^{(k)}$ is the image with the top $k$ most important features replaced. A steeper drop (higher AUC) indicates that the attribution method correctly identified the features most critical to the model's prediction. The Quantus implementation groups features into steps of configurable size (\texttt{features\_in\_step}), making the metric tractable for high-resolution inputs \cite{hedstrom2023quantus}.
...
Max-Sensitivity, proposed by Yeh et al., measures the worst-case explanation change under small random perturbations of the input \cite{MISSING-yeh2019}.
\studentinline{VERIFY citation: Yeh et al.\ 2019 --- ``On the (In)fidelity and Sensitivity of Explanations.''}
Given $N$ perturbation samples $\delta_1, \dots, \delta_N$ drawn from $\mathcal{N}(0, \sigma^2 I)$ with $\|\delta_j\| \leq \varepsilon$:
\begin{equation}
    \text{MaxSens}(\Phi, x) = \max_{j = 1, \dots, N} \frac{\|\Phi(x + \delta_j) - \Phi(x)\|}{\|\delta_j\|}.
\end{equation}
...
\begin{equation}
    \text{AvgSens}(\Phi, x) = \frac{1}{N} \sum_{j=1}^{N} \frac{\|\Phi(x + \delta_j) - \Phi(x)\|}{\|\delta_j\|}.
\end{equation}
...
\end{equation}
where $f^{(\ell)}$ denotes the model with the top $\ell$ layers randomized and $\rho$ is the chosen correlation measure. Lower average correlation indicates a better method---one that is genuinely sensitive to the model's learned parameters. As discussed in Section~\ref{sec:crisis}, Guided Backpropagation and DeconvNet fail this test, producing nearly identical maps for trained and randomized models.
...
\end{equation}
The metric measures the absolute deviation from this axiom; lower is better. Completeness is only meaningful for methods that explicitly satisfy or approximate the completeness property by construction---specifically Integrated Gradients, DeepLift, and LRP. For other methods, the metric returns NaN and is excluded from aggregation.
...
a climate-science benchmark, and \citet{schwalbe2024systematic} demonstrated
systematic rank reversals between faithfulness operationalizations on a
large-scale computer vision benchmark. \citet{kadir2024evaluating} argue that
...
signal, regardless of its theoretical motivation. This meta-evaluation layer
is essential for the framework proposed in this thesis, as it provides a
principled criterion for the \emph{selection} of metrics in Phase~2,
...
\emph{Autoweighted} mechanism, in which weights are derived from each metric's
own ensemble score: explanations that agree more strongly with the ensemble
consensus contribute more heavily to the aggregated assessment. Formally, if
$\mathrm{ES}_k$ denotes the ensemble score assigned to the explanation by
metric $k$, then
$$
w_k \;=\; \frac{\mathrm{ES}_k}{\sum_{j} \mathrm{ES}_j},
$$
producing a data-adaptive convex combination in which dominant, trustworthy
signals receive higher weight without manual tuning. In this thesis,
$\mathrm{ES}_k$ denotes the MetaQuantus-discounted ensemble score rather than
the raw ensemble score: each metric's ensemble agreement is first attenuated
by its meta-evaluation reliability (Noise Resilience and Adversarial
Reactivity from \citet{hedstrom2024metaquantus}), so the Autoweighted formula
already incorporates the meta-evaluation criterion into the weight derivation.
This mechanism provides the theoretical foundation for the evaluation framework
proposed in Chapter~\ref{ch:methodology}, which extends the Autoweighted
concept by jointly accounting for ensemble disagreement and MetaQuantus
failure rate in a single weighting step.

Recent work has emphasized that evaluation pipelines are themselves a
compliance instrument. \citet{hedstrom2025bridging} argue that standardized,
