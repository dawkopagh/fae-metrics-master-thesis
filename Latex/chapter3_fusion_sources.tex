...
the remaining weights renormalise so that $\sum_k w_k^{\mathrm{AW}} = 1$. The
coefficient of variation is scale-invariant, so Autoweighted yields the same
result whether computed on raw or Second-Moment-scaled scores.
...
an Adversarial Reactivity score $\mathrm{AR}_k$. Their combination is the
per-metric \texttt{combined\_reliability} reported in
\texttt{results/meta\_evaluation\_reliability.csv}, averaged over the
(model, FAE) cells in which the metric is defined.
...
Where \texttt{combined\_reliability} is nonetheless undefined (the
structurally excluded metrics), $\lambda_k$ is resolved by an
...
\begin{description}
    ...
    (Section~\ref{sec:metrics}); a strictly within-category policy is the
