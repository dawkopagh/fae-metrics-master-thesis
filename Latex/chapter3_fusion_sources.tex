...
\noindent
With these two exclusions, eleven metrics are computed per attribution triple
...
metric $k$, let $\mathrm{cv}_k = \mathrm{std}(s_k) / |\mathrm{mean}(s_k)|$ be
...
The novel weighting scheme of this thesis (Decision~D3) combines two signals
...
that internally re-compute attributions (the sensitivity metrics, MPRT, and
RandomLogit) for which the Adversarial-Reactivity test is methodologically
inapplicable; (ii)~the mean reliability of the other valid metrics in the same
