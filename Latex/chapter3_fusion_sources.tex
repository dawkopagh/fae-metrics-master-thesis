...
(model, FAE) cells in which the metric is defined. Where
\texttt{combined\_reliability} is undefined, $\lambda_k$ is resolved by an
explicit fallback hierarchy: (i)~the Noise-Resilience-only score, for metrics
whose Adversarial-Reactivity leg is unavailable (in this run,
\emph{max\_sensitivity} and \emph{random\_logit}, whose AR test---which
internally re-computes attributions---did not complete; both resolve to
$\mathrm{NR} = 1.000$ and are therefore effectively undiscounted);
