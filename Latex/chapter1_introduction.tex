...
\begin{itemize}
    ...
    \item \textbf{Chapter 4: Experiments and Results} – Presents the empirical validation of the proposed framework on the ISIC~2017 dermoscopic dataset \cite{sangwan2024quantifying}, whose expert-annotated lesion masks enable Localization metrics. It reports the metric redundancy analysis, the MetaQuantus-based reliability of each metric, the comparison of uniform, Autoweighted, and MetaQuantus-discounted aggregation, the contrast between individual and NormEnsembleXAI-ensembled attributions \cite{hryniewska2024normensemblexai}, and the statistical significance of the resulting rankings via Friedman, Nemenyi, and Wilcoxon testing.

    \item \textbf{Chapter 5: Discussion} – Interprets the rankings, examines the threats to validity (notably the sample-size limitation of the redundancy and reliability estimates), and delimits the scope within which the conclusions generalise.

    \item \textbf{Chapter 6: Conclusions and Future Work} – Summarizes the findings against each contribution, discusses the trade-offs between interpretability and performance \cite{doshi2017rigorous} and the computational limitations \cite{hryniewska2024normensemblexai}, and outlines future research into automated validation and regulatory compliance within the scope of the EU AI Act.
\end{itemize}
