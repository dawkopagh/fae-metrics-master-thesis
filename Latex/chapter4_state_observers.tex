...
point of use: (i)~the reliability of the two $M^{*}$
metrics that internally re-compute attributions (\emph{max\_sensitivity},
\emph{random\_logit}) rests on their Adversarial Reactivity alone: their
Noise-Resilience measurements failed silently in the meta-evaluation run
and are discarded as artifacts, and scoring their AR required an extension
...
carries a measured reliability, and the MQ-discount substantially
reweights the index (mean $|\Delta|\approx0.33$ on ResNet-18 and $0.16$ on
...
metric battery rather than of the classifier. One caveat on that identity
...
$|\rho|$-with-the-remainder of $0.323$ vs.\ $0.325$), so a marginally
different draw of images could have flipped the retained member on one
model and broken the cross-model identity; a repeated-subsample stability
check of this tie-break is deferred to future work. The pruning costs no
...
An independent audit of the reliability CSV found that the Noise-Resilience
values recorded for the explanation-recomputing metrics
...
cause is a defect in the NR implementation: when every seeded metric call
fails (as those metrics' calls did in the meta-evaluation run, consistent
...
total stability. The defect is fixed in the code (failed measurements now
yield NaN), the artifact NR values are invalidated in the released data, and
the combined reliability of the two affected $M^{*}$ metrics is therefore
their \emph{measured Adversarial Reactivity alone}
($\lambda_k = \mathrm{AR}_k$), reported as such in Table~\ref{tab:meta}.
Re-measuring NR for these metrics on Colab is prepared as follow-up work;
until then, no fabricated number enters the discount.
...
($r_k = 0.97$), \emph{max\_sensitivity} ($0.95$, AR-only), and
\emph{pixel\_flipping}
($0.88$) are the most reliable; \emph{pointing\_game} ($0.81$) follows.
\emph{Random\_logit} is now the \emph{least} reliable metric of all
($r_k = 0.27$, AR-only): its measured reactivity sits at the noise floor
...
values span $0.05$--$0.98$; $0.24$ for random\_logit), whereas the most
reliable metrics are also the
most stable across cells ($\mathrm{SD} \approx 0.09$--$0.18$).
...
adversarial reactivity---and therefore its entire AR-only reliability---is
...
two headline moves in all seven replicates: LRP is \emph{first} and
Grad-CAM \emph{last} in every replicate, and both survive shifting the two
AR-only reliabilities by $\pm 0.2$ (twice the estimator's granularity at
five levels, and well above it at ten). The mid-order---Guided
Backpropagation, Occlusion, and Integrated Gradients in places two to
four---is \emph{not} stable under these perturbations. The robust statement
is therefore that the discount lifts LRP from fifth under Autoweighted to
first and demotes Grad-CAM to last on ResNet-18, while the middle of the
field should not be over-read.
...
$|\Delta|\approx0.33$ on ResNet-18 and $0.16$ on SqueezeNet relative to
Autoweighted, and largely inverts the ResNet-18
ordering ($\rho(\mathrm{AW},\mathrm{MQ}) = -0.14$; LRP rises from
fifth under Autoweighted to first) while altering
...
Section~\ref{sec:exp-meta}, the MQ-discount column applies a strong
reliability reweighting (mean $|\Delta|\approx0.33$ on ResNet-18, $0.16$ on
SqueezeNet, relative to Autoweighted):
it largely inverts the ResNet-18 ordering ($\rho = -0.14$) while altering
SqueezeNet only mildly ($\rho = 0.89$). The de-facto weight vectors that
...
dominated by sparseness ($w = 0.52$) and pixel-flipping ($0.20$), with
pointing-game at $0.15$, random-logit at $0.10$, and---owing to the
location-sensitivity of the inverse-CV weight documented in
Section~\ref{sec:weighting}---faithfulness-correlation at only $0.02$ and
...
metric index, and the comparison against the Single-FC baseline below
...
(Table~\ref{tab:ranking-resnet}) the winner itself is scheme-dependent:
Occlusion leads under uniform ($E = 0.164$) and Autoweighted ($0.123$)
weighting, but under the MQ-discount LRP takes first place ($0.498$, from
fifth under Autoweighted) with Occlusion third ($0.353$), while Grad-CAM
drops from second under uniform
weights ($E = 0.133$) to fourth under Autoweighted ($0.023$) and last under
the MQ-discount ($0.280$). The Spearman rank correlation between the
...
(Single-FC vs.\ MQ-discount $\rho = -0.18$, against $0.79$ on SqueezeNet,
with sixteen discordant pairs summed over
...
$\chi^2 = 830.2$ / $1571.3$; MQ-discount: $863.8$ / $1080.9$; likewise
...
rankings on ResNet-18 ($W = 0.11$--$0.24$ across schemes) and moderate
concordance on SqueezeNet ($0.30$--$0.46$)---the method differences are
