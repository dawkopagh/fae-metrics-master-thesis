...
completion for \emph{max\_sensitivity} and \emph{random\_logit} used a
fixed \textbf{12-image split-order sample} distinct from the 64-image draw,
so those two metrics' combined reliability mixes the two samples. The reliability estimates are correspondingly
...
\emph{random\_logit} ($0.72$) follow. \emph{Faithfulness\_correlation}
($0.58$) and
...
still earn credit; auditing the signed monotonicity of the 42 extended-AR
cells finds 4 such cells (three for random\_logit, one for
max\_sensitivity, with $|\rho_S| \le 0.7$) --- their contribution is
...
headline reordering in all seven replicates: LRP remains second and the
top-2 order is unchanged, so the MQ-discount finding of
Section~\ref{sec:exp-ranking} is not an artefact of any single reliability
cell. A harsher perturbation---shifting the two extension-measured AR
values by $\pm 0.2$, twice the estimator's granularity---preserves the
reordering but can move LRP between second and third place; the robust
statement is therefore that the discount lifts LRP from fourth into the
top three while Occlusion stays first, with the exact position contingent
...
a winsorized pooled scale---a third normalizer that trims the tails---the
SqueezeNet comparison turns significantly \emph{against} the ensemble, so
...
rankings on ResNet-18 ($W = 0.13$--$0.23$ across schemes) and moderate
concordance on SqueezeNet ($0.34$--$0.44$)---the method differences are
