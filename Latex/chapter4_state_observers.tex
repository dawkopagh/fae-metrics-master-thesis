...
combined reliability for five of the seven $M^{*}$ metrics; the two whose
adversarial-reactivity leg did not complete (\emph{max\_sensitivity},
\emph{random\_logit}) fall back to their Noise-Resilience-only reliability
($\mathrm{NR} = 1.000$ for both, i.e.\ effectively undiscounted; see
Section~\ref{sec:exp-meta}). The discount attenuates the effectiveness
magnitudes (mean $|\Delta|\approx0.02$ relative to Autoweighted) while
leaving the induced ranking unchanged; and (ii)~the attribution-ensembling
...
Section~\ref{sec:exp-ranking} therefore applies the combined-reliability
discount to the five validated metrics and falls back to the
Noise-Resilience-only reliability for the other two (the first rule of the
documented fallback hierarchy, rather than dropping them); since both have
$\mathrm{NR} = 1.000$, they are effectively undiscounted. The resulting
\texttt{effectiveness\_mqdiscount} column differs from the Autoweighted column
by a small margin (mean $|\Delta|\approx0.02$): the discount attenuates the
effectiveness magnitudes but leaves the induced ranking unchanged (identical
method order on both models, Table~\ref{tab:rank-stability}). Closing the
...
reliability discount (mean $|\Delta|\approx0.02$ relative to Autoweighted): it
attenuates the magnitudes while leaving the induced method order unchanged on
both models.
...
schemes ($E = 0.164$ uniform, $0.123$ Autoweighted, $0.109$ MQ-discount), but
the mid-field reorders substantially: Grad-CAM drops from second under uniform
weights ($E = 0.133$) to fourth under Autoweighted ($0.023$), while Saliency
and Integrated Gradients move up. The Spearman rank correlation between the
...
sharply of all on ResNet-18 (Single-FC vs.\ MQ-discount $\rho = 0.464$ on
ResNet-18, $0.893$ on SqueezeNet, with nine discordant pairs summed over both
models), which confirms that a one-metric ranking is not a safe proxy for the
...
position is most sensitive (second to fourth on ResNet-18). Second, the
...
$\chi^2 = 830.2$ / $1571.3$; MQ-discount: $878.5$ / $1412.2$; likewise
