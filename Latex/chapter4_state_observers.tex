...
classifier~$\to$~\texttt{Conv2d(512, 3, 1, 1)}). Both architectures and their
modified classification heads are described in Section~\ref{sec:models},
which also fixes the training protocol (25 epochs, SGD with momentum,
class-weighted cross-entropy, StepLR, fixed seed~42); this section restates
only what is needed to read the results that follow. Validation accuracy and
...
excluded on technical grounds rather than by preference: the Robustness
metrics re-invoke the explanation function for every perturbation sample,
which multiplies the surrogate-fitting cost of both estimators and -- because
that fitting is stochastic -- leaves the resulting sensitivity scores unstable
and uninterpretable, so the measurement would not mean what the metric claims.
Section~\ref{subsec:excluded-fae} states the argument in full; their
reintroduction is recorded as future work in Section~\ref{sec:concl-future}.
...
table in this chapter deterministically from that score table (all three
are in the project repository, \url{https://github.com/dawkopagh/fae-metrics-master-thesis}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/redundancy_bars.pdf}
    \caption{All 36 pairwise Spearman $|\rho|$ values of the nine metrics
    that enter the correlation analysis, sorted, per model, against the
    $|\rho| > 0.85$ pruning threshold (Decision~D4). Hatched bars are the
    two pairs pruned on each model; the four largest values are named. The
    threshold sits in a gap: no pair falls between $0.79$ and $0.93$ on
    ResNet-18 or between $0.76$ and $0.98$ on SqueezeNet, so any threshold
    from $0.80$ to $0.93$ yields the identical $M^{*}$ of
    Table~\ref{tab:mstar}. The nearest survivors are cross-category
    (random\_logit with sparseness and complexity) -- the case the pooled
    pruning of Section~\ref{subsec:redundancy-pruning} exists to catch; what
    a lower threshold would do to them is discussed in
    Section~\ref{sec:disc-redundancy}.}
    \label{fig:redundancy-bars}
\end{figure}
...
of max-sensitivity; Figure~\ref{fig:redundancy-bars} shows the two pruned
pairs against the 34 that survive). The resulting reduced set $M^{*}$ is \emph{identical} for
...
Figure~\ref{fig:reliability-bars} shows the two legs side by side, each AR
bar against the noise floor of its own protocol.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/reliability_bars.pdf}
    \caption{Noise Resilience and Adversarial Reactivity of the seven $M^{*}$
    metrics (mean over the 14 (model, FAE) cells; the between-cell dispersion
    is tabulated in Table~\ref{tab:meta}), ordered by combined reliability
    $r_k$. The short black
    bar over each AR column is the expected $|\rho_S|$ of a random ordering
    under that metric's degradation protocol -- exactly $0.42$ for the
    five-level protocol, ${\approx}0.27$ for the ten-level extended-AR
    protocol of max\_sensitivity and random\_logit. The two patterns the
    text relies on are visible at once: random\_logit's near-perfect NR
    paired with an AR at its floor (stable but insensitive), and
    relevance\_mass\_accuracy's AR below its floor. The NR values of
    sparseness, pixel\_flipping, pointing\_game, and random\_logit are
    additionally bounded by the independence reference discussed below and
    should be read as upper bounds.}
    \label{fig:reliability-bars}
\end{figure}
