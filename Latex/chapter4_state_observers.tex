...
($r_k = 0.969$), \emph{pixel\_flipping} ($0.883$), and \emph{pointing\_game}
($0.811$) are the most reliable, combining high noise resilience with strong
adversarial reactivity. \emph{Faithfulness\_correlation} ($0.582$) and
...
Autoweighted per-method orderings is only $\rho = 0.286$ for ResNet-18, which
contributes nine of the ten discordant method-pairs that
Table~\ref{tab:rank-stability} reports summed over both models. SqueezeNet is
far more stable (Table~\ref{tab:ranking-squeeze})---Grad-CAM is first under
both schemes ($E = 0.360$ uniform, $0.250$ Autoweighted) and the
uniform/Autoweighted orderings agree closely ($\rho = 0.964$). The
single-metric baseline (Faithfulness Correlation alone) diverges most sharply
of all on ResNet-18, reordering the methods almost entirely
(Single-FC vs.\ MQ-discount $\rho = 0.286$ on ResNet-18, $0.893$ on
SqueezeNet, with eleven discordant pairs summed over both models), which
confirms that a one-metric ranking is not a safe proxy for
